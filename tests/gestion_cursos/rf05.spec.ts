...
// Empezar los tests
test.only('rf05-01 (Visualizar la lista de cursos eliminados) [T de estados]', async ({ page }) => {
    const contador = { valor: 0 };
    const casosPrueba = "rf05-01";
    ...
    const expandBtn = page.locator('#deleted-table-heading button.chevron');
...
test.only('rf05-02 (Restaurar curso eliminado) [T de estados]', async ({ page }) => {
    const contador = { valor: 0 };
    const casosPrueba = "rf05-02";
...
test.only('rf05-03 (Mostrar la alerta correctamente después de intentar eliminar el curso) [T de estados]', async ({ page }) => {
    const contador = { valor: 0 };
    const casosPrueba = "rf05-03";
...
test.only('rf05-04 (Eliminar un curso archivadoo) [T de estados]', async ({ page }) => {
    const contador = { valor: 0 };
    const casosPrueba = "rf05-04";
    ...
    const mensaje = await page.locator('div.toast-body').innerText();
    await Guardar_imagen(page, carpetaBase, contador, casosPrueba);
    expect(mensaje).toContain('has been permanently deleted');     
});

test.only('rf05-05 (Cancelar la eliminación de un curso archivado) [T de estados]', async ({ page }) => {
    const contador = { valor: 0 };
    const casosPrueba = "rf05-05";
    ...
    await Guardar_imagen(page, carpetaBase, contador, casosPrueba);   
    await expect(page.locator('#btn-delete-0')).toBeVisible();
    const courseId = await page.locator('#deleted-course-id-0').innerText();
    await page.click('#btn-delete-0');
